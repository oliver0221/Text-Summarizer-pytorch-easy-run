...
def shuffle_text_data(unshuffled_art, unshuffled_abs, shuffled_art,
                      shuffled_abs):
    article_itr = open(os.path.join(unfinished_path, unshuffled_art), "r", encoding='utf8')
    abstract_itr = open(os.path.join(unfinished_path, unshuffled_abs), "r", encoding='utf8')
    list_of_pairs = []
    ...
    article_itr = open(os.path.join(unfinished_path, shuffled_art), "w", encoding='utf8')
    abstract_itr = open(os.path.join(unfinished_path, shuffled_abs), "w", encoding='utf8')
...
def write_to_bin(article_path, abstract_path, out_file, vocab_counter=None):
    ...
    with open(out_file, 'wb') as writer:
        ...
        article_itr = open(article_path, 'r', encoding='utf8')
        abstract_itr = open(abstract_path, 'r', encoding='utf8')
    ...
    if vocab_counter is not None:
        with open(vocab_path, 'w', encoding='utf8') as writer:
...
if __name__ == "__main__":
    for file in ['eval', 'test', 'train']:
        df = pd.read_csv('./PreLCSTS/%s.csv' % file, header=None,encoding="utf-8")
        if file == 'eval':
            file = 'valid'
        with open('./data/unfinished/%s.title.txt' % file, 'w',encoding="utf-8") as f:
            for data in tqdm(df[0].tolist()):
                #f.write(preprocess(data).replace(u'\xa0', u''))
